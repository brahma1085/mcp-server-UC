...
// Global rate limiter for Google Docs APIs to prevent 429 errors.
// 5 concurrent requests maximum, at least 200ms apart.
const docsRateLimiter = new Bottleneck({
  maxConcurrent: 5,
  minTime: 200,
});
  ...
  async appendContent(
    ...
    const nextLock = previousLock.then(async () => {
      try {
        ...
        const doc = await docsRateLimiter.schedule(() =>
        ...
        await docsRateLimiter.schedule(() =>
