...
import { GoogleDocsService, ContentBlock } from '../services/google_docs_service';
...
export async function handleGoogleDocsTool(
  ...
  try {
    if (toolName === 'google_docs_append_content') {
      await docsService.appendContent(args.documentId, args.blocks as ContentBlock[]);
      return {
        content: [{ type: 'text', text: `Successfully appended content to Google Doc: ${args.documentId}` }]
      };
    }
